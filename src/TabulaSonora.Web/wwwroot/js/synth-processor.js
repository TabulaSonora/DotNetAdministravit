...
//
// .NET renders ahead of playback and posts blocks here; this drains them at whatever rate the
...
class SynthProcessor extends AudioWorkletProcessor {
    ...
    receive(message) {
        if (message.command === 'push') {
            // One buffer, left channel then right, as it came off the .NET heap.
    ...
    // messages a second; every 10 ms is frequent enough to hold the 64 ms lead that live playing
    // runs at, which is the tightest thing the pump has to keep from starving.
    report() {
        ...
        this.reported = now;
        this.port.postMessage({ queued: this.available, starved: this.starved });
    }
}
