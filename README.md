...
**Almost nothing Roland-derived is committed, and the one exception is named.** The library assembly
embeds exactly one data file, `Rom/manifest.json` — the offset *map*, not the data, which the upstream
spec repo tracks for the same reason. The extracted tables, the decoded audio and the test fixtures
are all generated locally from your own DLL and are gitignored.

The exception is `src/TabulaSonora/Effects/presets.json`, 27 KB of reverb and chorus coefficients,
which **is** committed. It cannot be regenerated by everyone who needs it — the engine computes those
numbers at start-up, so harvesting them means executing a Windows binary, which the browser build
cannot do. [`NOTICE.md`](NOTICE.md) sets out the reasoning and how to remove the file if you would
rather not carry it.

You need a legally obtained `SCCore.dll` from a Sound Canvas VA installation. The build pins one
exact file, the one that ships with **SOUND Canvas VA 1.1.6**:
...
The release number is there so you know which installer to look in; it is not what identifies the
file. **The DLL carries no version resource at all**, so nothing can read 1.1.6 out of it and nothing
verifies it — the hash, the timestamp and the size are the identity, and 1.1.6 is the release those
belong to.
...
**A working `presets.json` is committed, so you do not need to run `prepare` to get effects.** Run it
if you want to regenerate the file from your own DLL and confirm it matches. The library also honours
`TABULASONORA_PRESETS`, or a host can call `EffectPresets.Use(...)`; with no presets available at all,
the first effect render fails with a message saying how to fix it.

### One platform caveat, stated plainly

`prepare` needs **Windows x64** — but only for the reverb and chorus coefficients, and only once ever,
which is why the result is committed rather than left to each user.
...
binary. A browser cannot do it at all.
...
everywhere. Everything else `prepare` does works on any platform.
...
passage published with AOT measures **10.9×** and holds a full second of lead. The transport shows the
figure so this is never a matter of opinion. See
[the browser article](docs/articles/web.md).
...
| `src/TabulaSonora.Web` | the browser build — Blazor WebAssembly, audio out via an AudioWorklet |
